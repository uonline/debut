	...
	act = function()
		...
		if not _guild_camp_lord_greeting then
			...
			local question = [[
				...
				время. Жаловался, что тут для него стало слишком много народу. Искал уединения
				и спокойствия.
				^
				Блондин задумался.
				^
				...
				связываться с этим слюнявым урук. Попробуй разузнать у той суетливой женщины,
			...
			if _guild_camp_giveaway then
				...
				return greeting .. [[
					...
					^
					-- Похоже, старик пытался спрятаться, но и этого ему не удалось, --
					блондин показал тебе ряд ровных белых зубов, -- удачно, что ты его
					нашёл. Мне как раз нужно передать ему кое-что.
					^
					Он снимает со своего наряда небольшую брошь и передаёт тебе.
					^
					-- Уверен, это сдвинет твоё дело к бывшему советнику с мёртвой
					точки, -- ты принимаешь украшение.
					^
					-- Надеюсь, на это. Спасибо, -- ты раскланиваешься и уходишь.
				]];
			end;
		...
		-- Разговор
		return [[
			Ты подходишь к блондину, чтобы завязать разговор.
			^
			-- Не ожидал уведь здесь такого человека как ты.
			^
			...
			Прогонят имперцев, обнаружат, что всё прибрали к рукм синдикаты Вольных городов.
			^
