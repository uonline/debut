...
guild_camp_nameless = obj {
	...
	act = function()
		...
		return [[
			Урук окидывает тебя неожиданно внимательным взглядом.
			^
			-- Тебе не хватает меча, режимник, -- ворчит орк.
		]];
	end;
}
...
guild_camp_axe = obj {
	...
	act = [[
		Ты присматриваешься к топору орка. В его руках он кажется тебе
		игрушечным. Ты знаешь, что урук используют такие топоры как метательные,
		но ты врядли смог бы орудовать им одной рукой.
	]];

	inv = [[
		Ты рассматриваешь топор.
		...
	]];
}
