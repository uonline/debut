...
_guild_camp_knight_story = false
...
guild_camp_knight = obj {
	...
	act = function()
		...
		if _guild_camp_hadsex then
			...
			-- Упоминаем гадалку?
			if _guild_camp_knight_story then
				_guild_camp_knight_story = false
				...
				return [[
					...
					^
					-- Проклятье? Не слышал такого, -- внезапно суровость возвращается
					к воину.
					^
					-- Она упоминала какую-то башню.
					^
					-- Ночь в башне, проклятье и странное пророчество, -- он неожиданно
					поднимается на ноги скрипя доспехом, -- чертовщина какая-то. Но что-то
					в этом есть. Каждый раз когда речь заходит об этой башне лучше
					всё проверить.
					^
					Уходит.
				]]
		...
		if _guild_camp_horny then
			...
			return [[
				...
				^
				-- Кстати, насчёт неё. Она просила кое о чём попросить тебя.
				^
				...
				лица. Ты отрицательно качаешь головой. Неожиданно для тебя воин начинает смеяться.
				^
