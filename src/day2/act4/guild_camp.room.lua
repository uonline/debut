...
guild_camp_smuggler = obj {
	...
	act = function()
		...
		return [[
				^-- Слушай, а ты чем торгуешь-то.
				^-- Ну ты чё как этот, заправской режимник-то? Чем надо тем и торгую.
				^-- Ну извини, просто поинтересовался. Я вообще сам недавно завербовался
				если вам так режимники не нравятся, то где вы их до сих пор не толкнули?
				^
	...
	act = function()
		...
		if not _guild_camp_lord_greeting then
			...
			return [[
				...
				договорился о чём-то Урфангом. Но я не знаю деталей.
				^
				-- ...
				^
