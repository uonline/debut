...
guild_camp_knight = obj {
	...
	act = function()
		...
		if _guild_camp_hadsex then
			...
			return [[
				Ты подходишь к рыцарю. Он встречает тебя усталым взглядом.
				^
				-- Ей ещё что-то нужно?
				^
				...
				с чудовищами и захватчиками. Что сказать,
				...
				чудотворные дела. В силу меча и доблесть веры больше нет.
				От рыцарства уцелел лишь титул, которым тебя может наделить
				какой-нибудь толстозадый лорд. Впрочем, некоторым не нужен и он.
				...
				которые ты совершаешь, а не титул. Хоть мой титут ненастоящий,
				...
				стране. Помогаю простым людям в их бедах, таких как в этом городе,
				например. Ещё стараюсь собирать всевозможную информацию
				о рыцарях прошлого, о деяниях орденов. В этом здорово помогают бродячие
				старцы -- книжники. Они много знают об истории этой страны. А некоторые
				лелеют надежду, что когда-нибудь удасться восстановить Единый Орден.
				^
				...
				поднимается на ноги скрипя доспехом.
				^
				...
				по привычке проверил легко ли выходит меч, -- прощай.
				^
				Кивнув тебе, воин уходит.
			]]
		end;
		...
		if _guild_camp_horny then
			...
			return [[
				...
				^
				Ты благодаришь воина и покидаешь его.
			]];
		end;
	...
	act = function()
		...
		if _guild_camp_hadsex then
			...
			return [[
				...
				Test
				^
				-- Знаешь, вообще я ищу у вас старика-горбуна.
				^
				-- Хм. Он теперь не здесь. Урфанг стережёт его в соседнем тоннеле.
				^
