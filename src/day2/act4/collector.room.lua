...
collector_man_dlg = dlg {
	...
	phr = {
		-- Мрачная история Приграничья:
		{
			...
			[[
				Test
				^1. -- Мне сказали, что раньше вы входили в городской совет.
		...
		-- О катакомбах
		{
			...
			'Что это за место?';
			[[
				-- Пещера, в которой располагается вход в древние катакомбы.
				^
				-- Катакомбы? -- с тревогой в голосе спрашиваешь ты, -- я только сегодня узнал,
				что под городом есть канализация.
				^
				-- Весь этот проклятый континет изрыт вдоль и поперёк, -- ворчливо замечает
				бывший советник.
				^
				-- Эти катакомбы, -- спрашиваешь ты, -- кто их построил?
				^
				-- Мне-то откуда знать? -- бывший советник пожимает плечами, -- я не учёный.
				Почти каждый город Приграничья строили на каких-то руинах, чёрт знает чьих.
				До нашего появления Лианара была густозаселёна.
				^
				-- 
