...
collector_man = obj {
	...
	act = function()
		...
		local collector_read = [[
			...
			старик, -- и старость чудесным образом вернула ему остроту. Давай
			их сюда.
			^
			Ты извлекаешь из-под нагрудника документы и передаёшь старику.
			^
			-- Стой здесь, -- строго говорит он.
			^
			...
			^
			-- Так что мне делать? -- недоуменно спрашиваешь ты.
			^
			...
			мог. У меня остались ключи от некоторых дверей в башне, в том числе
			от его комнаты. Теперь они у тебя. Удачи.
			^
		...
		if have 'lord_sign' then
			...
			return collectors_drink .. [[
				...
				чтобы я отдал тебе одну вещь. Сейчас я подойду ближе и передам
				её.
				^
		...
		return [[
			...
			^
			-- Кто тут?! Я велел этому уруку никого не впускать!
			^
			-- Я его сменил. У меня к тебе дело, старик.
			^
...
collector_man_dlg = dlg {
	...
	hideinv = true;
	entered = function()
		-- Теперь мы сможем узнать старика на картине в башне
		event 'recognize old man';
		return [[
	...
	phr = {
		-- О катакомбах
		{
			...
			'Что это за место?';
			[[
				-- Пещера со входом в древние катакомбы, -- спокойно
				объясняет старик.
				^
				...
				известно, кому они принадлежали. До появления человека Линара была густонаселённой.
				^
				...
				континент полностью. Мы можем лишь строить догадки об его прежних обитателях.
				^
				...
				со своей родины. Мы забываем, что в мире существует нечто, против чего
				человечество не выступит на равных.
				^
				-- Что это значит? -- в смятении спрашиваешь ты.
				^
				Несмотря на давящую обстановку, голодный зев пещеры и её свод,
				теряющийся во тьме, ты чувствуешь странную притягательность этого места.
				Тебе начинает казаться, что время здесь замерло в ожидании чего-то. Вот-вот
		...
		-- Мрачная история Приграничья
		{
			...
			[[
				...
				закрутились все эти события. Я говорю про те события, что привели
				к Корадорскому конфликту.
				^
				Ты чувствуешь, как у тебя на лбу выступил холодный пот. В армии про
				...
				просто пропадали. В городах и деревнях. Их поиски не давали результата.
				...
				^
				-- Народ начал обвинять во всём корадорскую секту. Правители соседних городов быстро
				...
				очнулся от своего забытья, -- и для нас так и осталось загадкой, что стало
		...
		-- О культах
		{
			...
			[[
				...
				выражение -- смесь горечи и решимости.
				^
				...
				Возможно, то как всё сложилось -- лучший из возможных вариантов. Самый
				чудовищный страх человек может испытать, когда рушатся его представления о
				мире. Эта скорлупа, за которой мы прячемся от реальности. Моя тогда дала
				трещину. И теперь я лишён покоя, кажется, до конца своих дней.
				Думаю, что умру, так и не узнав истины. И это почему-то
				пугает меня. Словно что-то таится рядом со мной, так и оставаясь невидимым.
				^
				-- Уверен, всё бы объяснилось, -- говоришь ты, но уверенности в
				твоём голосе не чувствуется.
				^
				-- Объяснилось? -- старик качает головой. -- Объяснения, здравый смысл,
				логика и науки -- всё это лишь часть нашей скорлупы представлений.
				Ими мы отгораживаемся от действительности. Разве ты сам никогда не
				чувствовал фальшивость видимого? Там наверху -- залитый солнцем городской
				пейзаж со снующими жителями. Протяни руку и эта искусная картина упадёт,
				обнажив нечто настоящее.
				^
				Заметив растерянность на твоём лице, старик натянуто улыбается:
				^
				-- В чём я решительно уверен, так это в том, что кто-то определённо
				достиг своих целей. Многие нагрели руки на событиях в Корадоре и тем
				самым помешали нам докопаться до истины.
				^
				Ты ловишь себя на мысли, что тебе уже давно хочется убраться отсюда,
				оставив этого старика наедине с его тьмой. Но разыгравшееся
				воображение удерживает тебя. Почему-то кажется, что безопасней
				продолжать эту изнуряющую беседу, чем выйти за границу света факела.
				^
				-- Вы думаете, если бы лордам хватило мудрости, они бы не начали эту
				войну? -- спрашиваешь ты.
				^
				...
				На деле же государи -- обыкновенные люди: либо живут в мире собственных иллюзий,
				либо кончают безумием. Ни тот, ни другой путь не ведёт страну
				к лучшей доле.
				^
				-- А тот богоизбранный, -- вспоминаешь ты, -- что с ним стало? Возможно, ему
				удалось приблизиться к разгадке?
				^
				Лицо старика скривилось.
				^
				-- Режим кичится своими богоизбранными, но неизвестно ещё, насколько они
				преданы ему. Слухи об исчезновении некоторых из них появляются с
				завидным постоянством. Джэйрана пропал ещё до начала войны. И многим
				это показалось подозрительным. Я кое-что узнал о нём. Его силу богоизбранного
				выявили, когда он был уже в преклонных годах.
				На тот момент он считался учёным мужем, богословом. Его знания были велики,
				и став богоизбранным, он решил их приумножить. Он отправился в Приграничье
				и начал изучать здешние религиозные течения. Режим весьма волновала причина
				такого обильного появления культов. Джэйрана стал достаточно известен в Приграничье.
				Много странствовал, исследовал историю и культуру. Даже собирал местные
				легенды. Я думаю, что это знание сыграло с ним злую шутку.
				Он исчез вместе с Корадором, и, кажется, Режим с этим смирился.
				Но иногда я получал смутные свидетельства, что он ещё жив. Ходили слухи,
				что его видели в разных мелких поселениях в окрестностях
				Корадора. Люди узнавали старика со странным блеском в глазах,
				что раньше собирал истории. Интересно, чем он занимается теперь.
				^
				-- Что случилось с той корадорской сектой? -- спрашиваешь ты неожиданно для себя.
				-- Вы говорили, что её последователи объявлялись в других городах. Их всех
				отловили?
				^
				-- Cомневаюсь. Едва началась война, как о Корадоре с его сектой забыли.
				Да и всех последователей уничтожить очень сложно. Некоторые люди настолько
				привыкают быть частью чего-то, что даже после распада одного сообщества они
				готовы примкнуть к любому другому схожего толка. История знает примеры,
				когда верхушку культа полностью уничтожали, но со временем его прежние
				последователи объединялись вокруг нового покровителя. После войны можно
				было услышать истории о целых поселениях культистов под видом обычных деревень.
				Кто знает -- может, в одной из таких скрывается сейчас и Джэйрана.
				^
				Почему-то у тебя перед глазами всплывает староста твоей деревни. Ты вспоминаешь
				его странную улыбку, взгляд безумца и пространные речи. По твоему телу пробегает
				дрожь.
				^
				-- Вы говорите так, будто этот богоизбранный предал Режим.
				^
				-- Я действительно так думаю, -- отвечает бывший советник. -- Инквизиторы
				не просто так охотились на них двести лет назад. Богоизбранные обладают недоступной
				для обычного человека силой. А это, в свою очередь, ведёт к опасным соблазнам.
				Могущество идёт рука к об руку с желанием ещё большего могущества. В этом мире
				существует множество запретных способов достижения невероятной силы. Мне они
				неведомы, но все эти культы и секты прямое тому доказательство.
				^
				Старик медленно переводит взгляд в сторону чёрного жерла в чреве пещеры.
				^
				-- Магия -- лишь посредник. Наша жажда познания -- вот что приведёт человечество
				к гибели.
			]];
		};
		...
		-- Уходим
		{
			...
			function()
				...
				return [[
					-- Конечно.
					^
					Ты успеваешь запомнить непроницаемое лицо старика, прежде чем он гасит факел,
					погружая мир вокруг тебя во тьму. Ты оставляешь его наедине с этой тьмой и
					той незримой границей в иной мир, заключённой в причудливый орнамент.
				]];
			end;
		};
	};
