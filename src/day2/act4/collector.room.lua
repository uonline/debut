...
collector_man_dlg = dlg {
	...
	phr = {
		-- О катакомбах
		{
			...
			'Что это за место?';
			[[
				-- Пещера, в которой располагается вход в древние катакомбы, -- терпеливо
				объясняет старик, -- это все вопросы?
				^
				-- Катакомбы? -- с тревогой в голосе спрашиваешь ты, -- я только сегодня узнал,
				что под городом есть канализация.
				^
				-- Весь этот проклятый континет изрыт вдоль и поперёк, -- ворчливо замечает он,
				-- тут нечему удивляться.
				^
				-- Эти катакомбы, -- спрашиваешь ты, -- кто их построил?
				^
				-- Мне-то откуда знать? -- ты не чувствуешь раздражения в голосе старика,
				-- я не учёный. Почти каждый город Приграничья строили на каких-то руинах,
				чёрт знает чьих. До появления человека Лианара была густозаселёна.
				^
				-- Эльфами?
				^
				...
				континент полностью.
				^
				-- А зачем, если эльфы рассказали им всё что знали?
				^
				-- Ну это уж слишком, -- старик посмеивается, -- остроухие просто предупреждали,
				чтобы мы не совали нос в некоторые земли, например на север, где сами обитали.
				Что неудивительно. Но так же, например, они предостерегли наших предков осваивать
				Скалистый полуостров полностью. Как я понимаю, этот негласный закон до сих пор
				соблюдается в Тантикуле. Но, я не думаю, что они собирались поделиться с нами
				всеми своими знаниями. К тому же, кто знает, насколько правдивы были все их
				россказни. Может они и сами не знают всего о Линаре и мире.
				^
		...
		-- Мрачная история Приграничья
		{
			tag = 'about_conflict';
			false;
			'Мне сказали, что раньше вы входили в городской совет.';
			[[
				Test
				^^1. История Корадорского конфликта и заносчивость Режима:
					...
					^1.1. Приграничье как основной источник культов на человеческой Линаре:
