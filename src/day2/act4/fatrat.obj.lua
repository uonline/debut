...
fatrat = obj {
	nam = 'Крысы';
	dsc = [[
		Где-то совсем рядом с тобой копошатся {крысы}. Ты слышишь их мерзкий писк и радуешься,
		что они не голодны.
	]];
	tak = function()
		if _sewer_rathunt then
			return [[
				Ты осторожно крадёшься в темноте пытаясь по писку опеределить,
				где снуёт будущий ужин. Через пять минут безнадёжных зачерпываний
				горстей нечистот и всякого хлама, тебе, перепачканому по пояс,
				...
				бъёшь об него крысу. Когда писк прекращается, ты пряешь тушку за поясом.
				^
			...
			return [[
				...
				Тебе остаётся надееться, что крысы не начнут кусать тебя за ноги.
			]], false
		end
	end;
	inv = [[
