...
tunnel_leader = obj {
	...
	act = function()
		...
		about_exconsul = [[
			...
			крутя в руках связку ключей, -- возможно, это действительно так. Для многих
			в этом городе советник Конрой -- кость в горле.
			^
		]];
		...
		if _collector_drunk then
			about_tower_mission = [[
				-- Итак, мы тут задумали небольшое дельце, -- обращается он к тебе, --
				-- нужно пробраться в башню и найти там кое-кого.
				^
				-- Кого же? -- ты с опаской переводишь взгляд с девушки на главаря и обратно.
				^
				-- Одного советника, который ещё думает об этом городе, а не об интересах
				Режима или нашего дорогого лорда. Нашей парочке пригодился бы ещё один человек.
		...
		to_tower = [[
			...
			^
			Главарь решительно встаёт и нажимает что-то на боковой стене.
			...
			Стена у него за спиной отодвигается.
		]];
