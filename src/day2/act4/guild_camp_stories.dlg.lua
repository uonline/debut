	...
	entered = function()
		...
		if not _mage_greeting then
			...
			return [[
				...
				внимание. Необыкновенный блеск в его глазах словно толкает тебя вперёд.
				^
				Ты оказываешься напротив мужчины, и он закрывает свою книгу.
				^
				-- А вот и ты. Я знал, что у тебя всё получится, -- слышишь ты
				подозрительно знакомый голос.
				^
				-- Да-да, мы уже встречались сегодня, -- он усмехается, -- мы
				...
				-- он пожимает плечами, -- ты вроде сомневался, стоит ли тебе
	...
	phr = {
		-- 1. Спрашиваешь какие у мага дела в подполье
		{
			...
			[[
				...
				много возможностей, --
				маг хитро щурится, -- например, попасть куда-нибудь
				незамеченным. Или раздобыть что-нибудь редкое.
				^
			...
			function()
				...
				if _mage_dialogs then
					...
					-- Отключаем ветку разговора о возрасте мага
		...
		-- 1.1. Про синдикат
		{
			...
			[[
				...
				^
				Ты задумываешься, и догадка не заставляет себя ждать.
				^
				-- Cиндикаты Вольных городов, вот кто. Ты работаешь на один из них.
				^
				-- Вся жизнь -- игра теней, -- на лице мага играет довольная улыбка,
				-- и наконец-то ты рассмотрел мою.
				^
				...
				в сторону усача с картой, -- желающих поучаствовать найдётся много.
				^
				...
				Он весьма серьёзно настроен выдавить Режим из этого города и Приграничья
				...
				дотянуться своими щупальцами до Режима и Полисов.
				^
		...
		-- 1.1.1. Диалог о главном герое
		{
			...
			[[
				...
				^
				Тот, в свою очередь, задумчиво потирает подбородок.
				^
				-- Это сложно объяснить обычному человеку. Колдуны способны чувствовать магию
				в других существах и определять, способно оно к колдовству или нет.
				...
				прошлом нашего мира или магии. Откуда в тебе эта тяга к знаниям? Словно ты долго
				спал и после недавнего пробуждения хочешь выяснить, в каком мире ты теперь живёшь.
				^
				Его пронзительный взгляд по-прежнему направлен на тебя. Но сейчас тебе кажется,
				будто он смотрит сквозь тебя, пытаясь разглядеть что-то сквозь плоть.
		...
		-- 2. Обращаешь внимание на возраст мага
		{
			...
			[[
				...
				-- человеческая память весьма ограничена. Говорят, они вынуждены переносить
		...
		-- 2.1. Об инквизиции
		{
			...
			[[
				...
				действительно, могут быть весьма опасны. Поверь мне как студиозу, как
				книжному червю. Я читал записи многих магов, и некоторые из них переходят
				определённую грань разумного в своих рассуждениях. Настоящие маги -- свободные
				...
				исследовательский пыл в узде, правительство Магоса даже организовало
		...
		-- 2.1.1 О религии
		{
			...
			[[
				...
				недолюбливают магов. И я бы заметил, что это оправданно. Сами богословы
				иногда мыслят весьма опасно. Та же религия Режима с её единым богом, прибирающим
				к себе души праведников, а души грешников отправляющим в небытие. По мановению
				руки были вычеркнуты представления об аде и его демонах с их бесконечными муками,
				а заодно и несколько вех истории человечества. Поколения людей посвящали себя
				...
				пытались искоренить грех и боролись с сектами нечестивцев,
