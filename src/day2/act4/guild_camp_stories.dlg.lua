	...
	phr = {
		...
		-- 1.1.1. Диалог о главном герое
		{
			...
			[[
				...
				свечение в его глазах, -- имеет место какое-то внешнее вмешательство. Это
				интересный случай, который я бы с удовольствием исследовал.
				^
				...
				мечтательно выражение на пару мгновений.
				^
				-- Очень жаль, что ты отказываешься, -- с досадой продолжает маг, -- тебя можно
				назвать внимательным слушателем. Ты был бы хорошим подопытным. Возможно мы бы
		...
		-- 2. Обращаешь внимание на возраст мага
		{
			...
			[[
				...
				^
				-- Другими словами, -- усмехаешься ты, -- страна управляется
				старыми маразматиками.
				^
		...
		-- 2.1. Об инквизиции
		{
			...
			[[
				...
				^
				-- Думаю да, Но поверь мне, как тому кто жил в Магосе, там есть чего бояться
				помимо этого. В ваших краях магия редкий гость, поэтому ты слабо представляшь,
				на что она действительно способна. Многие церковники, кстати, тоже. Чтобы держать
				исследователький пыл в узде, правительство Магоса организовало свою инквизицию
				-- Око. Оно же охотится на непокорных магов.
				^
				-- Маги...
			]]
			function()
