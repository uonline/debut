	...
	phr = {
		{
			...
			[[
				...
				-- это страна свободных людей. Кто бы здесь ни правил и какой
				бы захватчик ни вторгался в эти края. Приграничье терпит лишь
				...
				^
				-- Я и не сержусь, старик. Но только разве не увешан этот город
				флагами с книгой и древом?
				^
				-- Приграничье сменяло флаги бессчётное количество раз.
				...
				осенью. Нет, эти земли принадлежат другой силе и только рыцарские Ордена
				...
				^
				-- На тот момент они были объединены в одно государство -- Единый Орден.
		...
		};
		{
			...
			[[
				...
				^
				-- Бродячий учёный? -- удивлённо спрашиваешь ты. -- Вот уж не думал, что
				...
				-- седая голова качнулась от глубокого вздоха, -- но ныне эти времена позади.
				...
				городов, пострадавших во время войны. Режим назначает здесь править
				...
				^
				-- Это верно. Кстати говоря, эффективность Доказательства Кровью также
				весьма меня интересует. Но ты прав, этот город я выбрал не случайно. Всё дело
				...
				^
				Облака рассеялись, и теперь башня выглядит не такой уж мрачной.
				...
				что башня существует, ей удалось собрать
				...
				обличьи. Например, Корадорский конфликт -- событие, с которого началась
