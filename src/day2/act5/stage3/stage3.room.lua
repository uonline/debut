...
tower_stage3_chairs = obj {
	...
	act = [[
		...
		^
		Хотя тебя самого так и подмывает развалиться на одном
		из резных стульев и погрузиться в сладостный сон. Ты так устал за день,
		что даже голод отступил на второй план.
	]];
	used = function(self, what)
		if what == tower_spear then
		end;
...
tower_stage3_picture = obj {
	...
	act = [[
		Одна из картин привлекает твоё внимание. Взгляд с холста...
		Ты узнаешь на картине короля. Смерть известных личностей.
		Картина висит неровно.
	]];
	used = function(self, what)
		if what == tower_spear then
