...
tower_stage3_archive_book_2 = obj {
	...
	act = function()
		return [[
			...
			на востоке, но оружие и ювелирные изделия его производства высоко ценились по
			всему миру. Королевство вело торговлю и с эльфами. Группы отважных искателей
			сокровищ спускались в глубины Разломов, чтобы найти там неизвестные артефакты.
			...
			^
			Следующий десяток страниц намертво слипся, словно намеренно заклеен. Тебе приходится
			пропустить их, чтобы продолжить чтение.
			^
			...
			уничтожены, но исчезновение такого могущественного королевства остаётся самой
			большой загадкой тех лет.
			^
			легенды о чёрных птицах
			Недра разломов. Лихо. Проклятье. Черные птицы,
			король и забвение.
			^
			^
			Test^
			Ты решаешь рассмотреть вкладыш и находишь на нём ряд надписей убористым почерком.
