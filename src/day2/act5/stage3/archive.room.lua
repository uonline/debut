...
tower_stage3_archive_book_4 = obj {
	...
	act = function()
		...
		return [[
			...
			^
			В тоже время поведение Леса даёт основания полагать, что подобные вторжения
			гоблинов случались и раньше.
			...
			^
			Всё в них указывает, что это существа чужды нашему миру. Они больше годятся, для жизни
			под землёй, нежели под солнцем.
			^
			Вторжения гоблинов.
			Наземные, пещерные, подземные. Вырождение и деградация из-за жизни на земле. Описания
			гоблинов Сиверсом.
			О размножении гоблинов. Мутации гоблинов. Божество гоблинов.
