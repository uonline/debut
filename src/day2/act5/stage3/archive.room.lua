...
tower_stage3_archive_book_4 = obj {
	...
	act = function()
		...
		return [[
			Ты осторожно открываешь книгу на...
			^
			Думается мне, нет на свете человека, который бы не признал гоблинов самыми
			мерзкими и злобным тварями на земле. И всё же сегодня к ним относятся с
			...
			с эльфами, разбли гоблинские армии и изгнали их в холодные южные тундры.
			Реальные же исторические свидетельства, рисуют совершенно другую картину.
			Вот как описывает события тех лет ...:
			^
			...
			черные колдуны и громадные тролли. Были твари и вовсе неведомые. Ночью они подошли
			к Тантикулу и штурмовали город с ходу. Звёзды скрылись за тучами нетопырей, и кроме их
			писка не было слышно более ничего. Надежды не оставалось никакой, горы трупов едва сравнялись
			по высоте со стенами. Готовились трубить отступление, хотели уходить в горы,
			вглубь Скалистого полуострова к самому озеру Саарат,
			а может и дальше. Но к утру полчища рассеялись, словно их и не было. Не оставили
			даже трупов, все сожрали. Гоблины ушли на север. А неделей позже к городу прибыли
			послы из так называемого Леса, получившие позднее название эльфы..."
			^
			Из данного текста становится очевидным, что люди в те времена не были гоблинам
			реальными соперниками.
			Недооценённость угрозы эльфов.
			Беженцы против гоблинов. Помощь эльфов. Вторжения гоблинов.
			Наземные, пещерные, подземные. О размножении гоблинов. Мутации гоблинов. Божество гоблинов.
			Имена гоблинов.
