...
on_event('freedom for black bird', function()
	...
	tower_stage4_sentry_black_bird:enable();
end)
...
tower_stage4_sentry_window = obj {
	...
	act = [[
		...
		всё больше места на земле. Ночью они будут править здесь безраздельно.
		Горожане будто и не подозревающие о грозящей им опасности, деловито снуют по улицам.
		...
		У памятника каменному древу и книге собрал толпу толи глашатай, толи менестрель.
		^
	...
	act = function()
		if not have 'tower_stone' then
			...
			return [[
				...
				Камень с радостью приземляется обратно в твою ладонь. Повторив это действие
		...
		return [[
			Ты машинально раскидываешь ногой кучку обломков кладки на полу.
...
tower_spear = obj {
	...
	inv = [[
		...
		копьё такое лёгкое. Наконечник тупой и покрыт ржавчиной. Так себе оружие, но хоть
		другого тебе не найти.
	]];
	tak = [[
...
tower_spyglass_paper = obj {
	...
	tak = function()
		...
		tower_stage4_sentry_situation:enable(); -- включаем обстановку в караульной
		return [[
...
-- Обстановка в караульной
tower_stage4_sentry_situation = obj {
	nam = 'Обстановка в караульной';
	dsc = [[
		^
		Посреди комнатки стоит крохотный стол и пара стульев.
		На столике покоится чей-то давнишний обед: черствеет ломоть хлеба,
...
tower_stage4_sentry_situation:disable();
...
tower_stage4_sentry_black_bird = obj {
	...
	dsc = [[
		...
		под потолком. Она висит вниз голвой, наподобие летучей мыши, и поглядывает в твою
	...
	used = function(self,what)
		...
		if what == tower_spyglass then
			return [[
				...
				^
				Расправив крылья, птица неожиданно набрасывается на тебя.
				Огромная, словно штормовая волна, она накрывает тебя с головой,
				погружая в черноту своего оперенья.
				^
				Ты начинаешь размахивать руками, чтобы отбиться от острых перьев, но
				крылья уже превращаются в густой дым. Ты ощущаешь резкий привкус гари во рту.
				...
				^
				^
				Поток крови усиливает своё течение, словно огромное сердце толкает его по улицам-венам.
				...
				^
				^
				В последний раз ты вдыхаешь дым горящего города,
				и чёрная волна накрывает тебя с головой.
				^
				^
