...
tower_stage1_thieves_leader = obj {
	...
	act = [[
		...
		я не первый раз в башне.
		Учитывая слухи о ней, я рассчитывал сделать здесь схрон.
		Раньше у меня не было ключей. Но вот я здесь с ключами,
		но я снова в ступоре.
		^
		-- Что ты имеешь ввиду? -- удивляешься ты.
		^
		-- Сложно объяснить. Ты сам всё увидишь дальше.
		^
		-- Разве нам не надо пошевеливаться? Нас не схватят
		стражники?
		^
		-- Много стражи здесь не будет. Башня строилась предыдущим наместником,
		который исчёз после завершения её строительства. Нынешний наместник здесь
		никогда не жил. Он превратил в свою резидёнцию один из лучших постоялых дворов
		в городе. Кроме советника Конроя других важных шишек здесь нет. А его не особно
		торопятся охранять.
		^
		...
	]];
...
tower_halfblood = obj {
	...
	act = [[
		...
		каждый раз всё по-новому, -- ловит твой взгляд Полукровка.
		^
		-- И что ты здесь делала?
		^
		-- Пряталась, -- просто ответила девушка.
	]];
}
