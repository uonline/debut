...
burning_quarter_godchosen = obj {
	...
	act = function()
		return [[
			...
			Хоть это и древний старик, но он богоизбранный, при доспехах и при мече.
			И ещё он пережил нападение орков на эшафоте. И похоже, вышел победителем из
			схватки с главарём орочьей банды. У тебя нет шансов.
			^
			...
			^
			К чему все эти разговоры о правосудии, разве сейчас они к месту?
			^
			-- Я был обычном солдатом, для меня был только сражение и деньги.
			-- Но когда я стал богоизбранным...
		]];
	end;
	used = function(self, what)
...
burning_quarter_godchosen_down = obj {
	...
	dsc = [[
		^
		{Богоизбранный} поднялся с земли и застыл, стоя на одном колене.
