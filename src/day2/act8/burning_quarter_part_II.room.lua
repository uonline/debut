...
burning_quarter_godchosen_down = obj {
	...
	dsc = [[
		^
		{Богоизбранный} поднялся с земли и теперь устало опирается на меч.
	]];
	act = function()
		return [[
			Ты замечаешь, что его глаза начинают излучать приглушённый свет.
				Его старческие
	...
	used = function(self, what)
		...
		if (
			...
			walk 'burning_quarter_part_II_gameover';
			return [[
				в надежде, что падение не прошло бесследно ты выступаешь против латника.
				Будто натыкаешься на стену. Воздух словно сгустился, и твой удар кажется
				тебе невероятно долгим.
				Ты так и не успеваешь его завершить. Меч свистит в воздухе,
				и ты как-то странно валишься на бок, а на глаза опускается пелена.
				Ты не успел понять, что удар богоизбранного разрубил тебя пополам.
			]];
		end;
	end;
}
