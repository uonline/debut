...
burning_quarter_mad_strike = function()
	...
	if (
		...
		(not have(burning_quarter_fight_hammer)) and
	) then
		mad_strike_text = [[
...
burning_quarter_dagger = obj {
	...
	used = function(self, what)
		...
		local priest_death_text = [[
			...
			и перед тобой оказывается закованный в броню воин с мечом в руке.
			^
			-- Ты дождался меня, дезертир, -- спокойно произносит богоизбранный.
		]];
		if what == burning_quarter_halberd then
			priest_death_text = [[
				Ты набрасываешься на проповедника с алебардой наперевес.
