	...
	phr = {
		...
		-- Диалог с магом крысой
		{
			...
			[[
				...
				идентичности. Нужно отдать должное архимагам Магоса. Эти старики осмелились
				иcпользовать головы других людей как коробку для собственных воспоминаний, мало заботясь
				о сохраности целостности своей личности. Интересно, куда их это приведёт...
				^
				Ты пытаешься найти изъян в лице беглого мага, но он выглядит так же
				как в лагере подполья.
				^
				-- Теперь-то ты понял, -- он говорит так, будто читает твои мысли,
				-- почему восприятие людей ограничено? В темноте человек слеп.
				Его можно только вести за руку.
				^
				Маг протягивает тебе руку и отступает во мрак, превращаясь в неподвижную
				фигуру-истукана. Лишь на месте его глаз, загорается два лиловых уголька.
				^
				-- Ты получил ответ на свой вопрос? -- спрашивает комната голосом Полукровки.
			]];
			function()
				black_room_she_dlg:pon('song_to_say_goodbye');
			end;
		...
		-- Переходим на эшафот для второй локации
		{
			...
			[[
				-- Всё что мне нужно, это чтобы ты помнил обо мне и о наших беседах,
				-- объявляет комната, -- помнил, не смотря ни на что.
				^
				Фигура совершает размытое движение, и чёрная штора опадает на пол.
				Свет врывается в команту.
				^
				Часто моргая, ты с удивлением осматриваешь покои советника, секунду
				назад бывшие чёрной комнатой.
				На столе перед тобой распростёрто бездыханное тело самого советника.
				Кровь стекает прямо на тебя, сидящего на кресле.
				^
				-- В этом мире всё так зыбко и преходяще, -- обращает на себя внимание Полукровка.
				В её руке красуется окровавленный нож.
				^
				-- Одно мгновение может разделить человека и страдание, человека и смерть.
				И он, в свою очередь, даже не сможет найти причину произошедшего.
				Перед глазами у него пронесётся вся его нелепая жизнь, а он так и не сможет осознать,
				почему же он умирает, -- девушка показывает тебе ровные белые зубы,
				-- о, воистину, мы не так представляем себе то, чего случиться
				...
				^
				Сам не понимая зачем, ты бросаешься к окну, чтобы камнем рухнуть вниз.
				^
			]];
			function()
				walk 'scaffold';
