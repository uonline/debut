	...
	phr = {
		...
		-- Диалог об идеях
		{
			...
			[[
				...
				-- а на самом деле, вы не так уж и отличатесь друг от друга.
				Вы состоите из плоти и крови. Дышите одним воздухом. Едите и спите.
				Вам накомы схожие эмоции. Вы способны чувствовать и мыслить.
				...
				^
				-- К тому, что ты до сих пор притягательно чист,
				-- призывно вытягивает голос Полукровки,
				-- ты стремишься лишь к существованию и выживанию. И ниодна
				идея ещё не пустила в тебе свои ростки, дезертир. Но приходит
				время, и тебе чем-то проникнуться.
				^
				-- Чем же я должен проникнуться? -- вопрос вырывается сам собой.
				^
				-- Пока это может оставаться тайной. Некоторые идеи остаются недосягаемы
				для сознания. В этом-то и залог их выживания.
			]];
			function()
				who_are_you_dialog();
			end;
		...
		-- Диалог с магом крысой
		{
			...
			[[
				...
				^
				Фигура вздрагивает и придвигается к свету свечи, и ты внезапно
				видишь перед собой лицо мага-крысы.
				^
				-- Тьма многолика, мой друг, -- произносит он как обычно довольный собой.
				^
				Теперь-то ты можешь признаться сам себе...
				^
		...
		-- Переходим на эшафот для второй локации
		{
			...
			[[
				...
				этого мира...
				^
				В его глазах загорается лиловый огонь.
				^
