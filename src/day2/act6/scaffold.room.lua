	...
	act = function()
		local text = {
			...
			[2] = [[
				...
				непонятную надежду. Изредка кто-то из них нет нет и обернётся.
				Бледность и глубокие тени под глазами сделают его лицо похожим
				на обтянутый кожей оскал черепа.
				Армия мертвецов...
				...
			]];
		};
	...
	act = function()
		local text = [[
			...
			* Настоящие боги этого мира это идеи. Эти сушества симбионты немыслимы без людей,
			...
			Ты был поржаён этой навязчивой идеей? Идеей спасения. Но куда она тебя привела? На эшафот!
			Многие идеи тщетны...
...
scaffold_godchosen = obj {
	...
	dsc = function()
		local text = {
			...
			[2] = [[
				^
				В центре эшафота рядом с плахой стоит {богоизбранный} и буднично протирает
				меч тряпицей. В его тени городской глашатай готовится зачитывать очередной
				приговор.
			]];
		};
	...
	act = function()
		return scaffold_action [[
			Ты смотришь на сурового латника. Странно наблюдать богоизбранного в
			качестве палача, да и ему самому эта роль, похоже, не в радость.
			Окровавленный меч. Ты уже видел его: его точил старый солдат в арсенале
			Режима. Что он там про него рассказывал? Трофейный меч отца здешнего лорда.
		]];
	end;
};
