	...
	act = function()
		local text = {
			...
			[2] = [[
				...
				Да разве дадут тебе что-то сказать?
			]];
		};
...
scaffold_propagandist = obj {
	...
	act = function()
		local text = [[
			...
			-- У нас еще есть шанс. Если вот-вот появятся орки,
			нам всего лишь нужно как-то потянуть время -- ты киваешь на толпу,
			-- давай, заболтай их как ты умеешь. Отпусти грехи или наоброт прокляни.
			^
			Глашатай Благих смотрит на тебя как на умалишённого.
			^
			-- Я предпочитаю определённость, -- спокойно говорит он, -- умереть здесь и сейчас от удара меча несравненно лучше, чем попасть в хаос мясорубки. Ты хочешь гадать, когда какой-нибудь орк переломает тебе все кости, выдернет руки с корнем или сожжёт заживо? Я не хочу.
			...
			Поздновато ты задумался о душе...
...
scaffold_godchosen = obj {
	...
	act = function()
		...
		if _scaffold_the_end_counter == 1 then
			text = [[
				Ты смотриь на латника. Тот ловит твой взгляд, и пару мгновений
				вы смотрите друг на друга. Про возраст.
			]];
		end;
