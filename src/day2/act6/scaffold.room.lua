...
scaffold_guards = obj {
	...
	dsc = [[
		...
		бросая косые взгляды то на толпу, то на заключённых. Некоторые
		нервно поглаживают эфесы мечей.
	]];
	act = function()
		return scfpick {
			[1] = [[
...
scaffold_propagandist_and_singer = obj {
	...
	dsc = 'Ближе всего к тебе оказываются двое... {Хмурые}';
	act = function()
...
scaffold_prison_guard = obj {
	...
	dsc = scfpick {
		[1] = [[
			Позади них ты видишь ещё пару своих знакомцев, но никак не можешь вспомнить,
			где ты его видел.
		]];
		[2] = [[
			{Тюремщик}.
...
scaffold_godchosen = obj {
	...
	dsc = [[
		Одного из них всё время пробивает дрожь, едва с эшафота
		раздаётся зычный голос городского глашатая.
		Следом за ним раздаётся скрежет стали и вой огромного меча.
