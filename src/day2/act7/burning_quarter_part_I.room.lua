...
burning_quarter_action = function(act_text)
	...
	if burning_quarter_counter == 3 then
		act_text = act_text .. [[
			...
			знобить.
			^
			-- Кто-то? -- громко удивляется богоизбранный, так чтобы ты услышал,
			-- это не кто-то, а приговорённый к смерти. И она неминуемо настигнет
			его.
			^
	...
	if burning_quarter_counter == 2 then
		act_text = act_text .. [[
			2...
			Орки врываются через другой проход...
		]];
	end;
...
burning_quarter_to_lane_fail_room = room {
	...
	enter = function()
		...
		if not burning_quarter_loaded_cart:disabled() then
			return [[
				Ты осматриваешь кучу бочек в кузове. Там явно нет места,
				так что забраться на повозку не выйдет.
			]], false;
		end;
