...
burning_quarter = room {
	...
	enter = function()
		...
		return [[
			...
			^
			Оказавшись в узком коридоре между зданий, ты умеряешь пыл.
			...
			^
			В ответ тебе открывается одна из дверей в ближайшем доме.
			На пороге высится чёрная громада орка в ореле приглушённого света.
			...
			Ты встаёшь и отправляешься на поиски своих противников.
			Наткнувшись на полуящера, ты хладнокровно всаживаешь в него обломок меча, до тех
			пор пока он не прекращает трепыхаться.
			^
			Переведя дыхание, ты ищешь орка. Глаза наконец-то привыкают к темноте, и
			распластавшееся тело обнаруживается у тебя под ногами.
			Оно покрыто сетью татуировок, которые ты, кажется, уже видел сегодня. ...
			Не обращя внимание на усилившиеся стоны из открытой двери, ...
			Ты подбираешь молот.
			^
			Шатаясь, ты идёшь к свету в конце переулка.
