...
burning_quarter_action = function(act_text)
	...
	if burning_quarter_counter == 3 then
		act_text = act_text .. [[
			3...
			Крики внизу квартала появляются тени.
			Богоизбранный и солдаты.
			^
			Один из соладт показывает в твою сторону.
			^
			-- Кто-то? -- говорит богоизбранный, так чтобы ты услышал,
			-- это преступник избежавший наказания. Но оно неминуемо настигнет
			его. Тебе не сбежать дезертир. Потому что бежать тебе не куда.
			Я тебя
			^
			-- Они нашли нас! -- кричит солдат.
			^
			-- Ну так к бою, -- приказывает богозибранный.
			^
			Отряд скрывается в соседнем проходе.
		]];
	end;
	...
	if burning_quarter_counter == 1 then
		act_text = act_text .. [[
			1...
			Орков побеждают и они отступают в проход. Передышка?
			^
			-- Я вернусь за тобой, дезертир, -- кричит тебе Кевраза.
		]];
	end;
