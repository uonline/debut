...
burning_quarter_action = function(act_text)
	...
	if burning_quarter_counter == 1 then
		act_text = act_text .. [[
			Ты прислушиваешься отголоскам боя в одном из проходов внизу квартала.
	...
	if burning_quarter_counter <= 0 then
		...
		return act_text .. [[
			Тени удилнняются и превращаются в богоизбранного
			в сопровождении двух солдат. Кажется они о чем-то спорят.
			Латник вырывает из рук солдат копьё и размахнувшись выпремляет руку.
			Копьё растворяется в воздухе. Вместо того чтобы бежать, ты всматриваешься в
			дым. И тут же неведомая сила опрокидывает тебя на землю.
			Руки твои бессысленно дёргают проклятое копьё, пригвоздящее тебя к земле.
		]];
	end;
...
burning_quarter_to_lane_fail_room = room {
	...
	enter = function()
		...
		if burning_quarter_cart:disabled() then
			...
			if burning_quarter_counter == 2 then
				fail_text = [[
					Раненный разоритель выбегает из прохода и стреляет
					в героя из лука.
				]];
			end;
