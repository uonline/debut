...
burning_quarter_cart = obj {
	...
	act = function()
		...
		if burning_quarter_corpses:disabled() then
			return text_start .. [[
				Заглянув под повозку, ты обнаружил несколько мертвецов.
			]] .. text_end;
		end;

		return text_start .. [[
				Заглянув под повозку, ты обнаружил ещё несколько мертвецов, помимо тех,
...
burning_quarter_cart_chain = obj {
	nam = 'Цепь повозки';
	dsc = [[
		Внушительная {цепь} удержает повозку от стремительного спуска со
		склона к самому большому из проходов.
	]];
	act = function()
		return [[
			Ты рассматриваешь цепь. Крепкая на вид, на деле
			она оказывается проржавешей.
		]]
	end;
...
burning_quarter_corpses = obj {
	...
	dsc = [[
		...
		Но больше всего тел около повозки. Здесь окоченело несколько
...
burning_quarter_salers_counter = obj {
	...
	act = function()
		return [[
			Ты разглядываешь палатку.
			Необычная, она украшена вольчими имедвежьими шкурами.
			Ты гадаешь как ей удалось уцелеть.
			Волчья шкура в крови... и перевёрнутый котелок...
			...
			Палатка на пути повозки к проходу.
		]];
	end;
	used = function(self,what)
