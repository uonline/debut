...
lane_room = room {
	...
	dsc = [[
		Описание переулка...
		В переулке... после горящей печи квартала кажется прохлада.
		на земле подрагивают жирные чёрные тени.
		влажный полумрак заглушает треск огня и горохот разрушений и ветра.
	]];
	obj = {
		'lane_covered_hatch';
	...
	enter = function()
		...
		return [[
			...
			Подрагивая, оно воткнулось прямо рядом с люком.
		]];
	end;
}
...
lane_covered_hatch = obj {
	nam = 'Люк';
	dsc = [[
		Совсем рядом {люк}, ведущий под землю.
	]];
	act = function()
	end;
	used = function(self, what)
		if what == lane_arms then
			walk 'killed_in_lane';
			return [[
				Ты хочешь открыть люк, но ничего не выходит.
				Кевраза тебя убивает.
			]];
		end;
	end;
}
...
lane_wall = obj {
	nam = 'Кладка стены';
	dsc = [[
		Слева от тебя на стене чернеет чёрная трещина-змея похожая на тень,
		но и то и дело отсветы пламени не могут её изгнать.
...
lane_huge_beam = obj {
	nam = 'Огромная балка';
	dsc = [[
		От разрушения стены балка... {Огромная балка} лежит...
	]];
	act = [[
...
lane_mystical_artifact = obj {
	nam = 'Амулет';
	inv = [[
		Ты осматриваешь амулет.
		Удивительно, но ты без труда можешь рассмотреть его в полумраке.
		^...
		Кость из которой выполнен миниатюрный карлик,
		неожиданно красуется металлическими бликами.
