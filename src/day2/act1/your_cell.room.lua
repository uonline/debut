...
your_cell = room {
	...
	enter = function()
		-- Если принесли тело стражника, то бросаем его
		if have 'prison_hall_guard_body' then
			event 'guard body is hidden';

			return [[
				Пыхтя, ты затаскиваешь стражника в свою камеру и с облегчением
				бросаешь тело на пол. Как безвольная кукла оно раскидывает руки
				и прекрасно изображает из себя измождённого заключённого.
			]];
		end;
	end;
	exit = function()
		...
		if not prison_cell_guard_body:disabled() then
			...
			if not have 'rat' then
				take 'rat';
				return [[
					Ты разворачиваешься, чтобы уйти, но резкий писк застигает тебя на пороге.
					Обернувшись ты видишь у своих ног крысу, не отрывающую от тебя глаз.
					Ты наклоняешься и протягиваешь руку.
					Грызун послушно запрыгивает в твою раскрытую ладонь и замирает.
					Пожав плечами, ты прячешь крысу за пазуху.
					^
					Ты покидаешь своё временное пристанище и запираешь камеру на ключ,
					оставив своего бывшего тюремщика, нести бремя пленника за тебя.
				]];
			end;

			return [[
				Ты покидаешь своё временное пристанище и запираешь камеру на ключ,
				оставив своего бывшего тюремщика, тянуть лямку заключённого за тебя.
			]];
		end;
	end;
}
	...
	act = function()
		if _wants_cocaine then
			...
			return [[
				...
				^
				-- Засранец снюханный, -- сквозь зубы цедит тюремщик.
				В голосе его слышится странная зависть,
				-- потом разберусь с тобой. Смотри только не сдохни.
				^
				...
				Устроившись поудобней ты ждёшь, сам не зная чего.
				Но ожидание оправдывает себя, когда из дыры появляется крыса с ключами в зубах.
				Не мешкая ни минуты, ты открываешь камеру.
			]];
		end;
...
prison_cell_guard_body = obj {
	...
	dsc = [[
		Посреди камеры лицом вниз лежит {стражник}.
	]];
	act = [[
		Ты осматриваешь неподвижное тело. Сразу и не скажешь, что это тюремщик,
		а не пленник.
	]];
};
