...
prison_hall_guard = obj {
	...
	dsc = function()
		if _guard_just_died then
			return [[
				У твоих ног лежит труп стражника.
			]]
		end
		return [[
			В десяти шагах от тебя {стражник} внимательно изучает план
			эвакуации при пожаре.
		]]
	end;
	act = [[
		Ты внимательно и с некоторым сочувствием рассматриваешь стражника.
		Его жизнь, должно быть, действительно очень скучна.
	]];
	used = function(self, what)
		if what == prison_hall_cup then
			...
			return [[
				...
				^
				Теперь надо куда-то оттащить труп. Может, вон за ту железную
				дверь?
			]]
		end
	end;
