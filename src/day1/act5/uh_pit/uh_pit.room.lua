...
uh_pit = room {
	...
	dsc = [[
		...
		закладывает уши.
		^
		Вместе с кровью в твоей голове яростно пульсируют воспоминания.
		В каждом лагере урук-хай, который ты видел, было это некое подобие арены --
		как правило, заваленное трупами.
...
pit_axe = obj {
	...
	tak = function()
		...
		if not _goblin_dead then
			...
			return [[
				...
				одичалый вгоняет гоблину в живот когти и швыряет его в сторону.
				Тушка гоблина несколько раз ударяется о землю и замирает без движения.
				^
				С рёвом одичалый обламывает торчащее из плеча древко копья.
				Банда эхом вторит ему.
			]];
		end;
	...
	inv = [[
		...
		низкокачественного железа. Напоминает тебе работу кузнеца.
	]];
}
...
pit_warren = obj {
	...
	act = function()
		...
		return [[
			...
			Теперь ты понимаешь, что деревенский староста имел в виду, когда
			сравнивал тебя с Ростовщиком. Вы оба были солдатами до того, как
		...
		if (what == pit_fists) or (what == pit_shield) or (what == pit_axe) then
			if have 'pit_shield' then
				...
				return [[
					...
					в глазах Ростовщика задёргивается пеленой. Гоблин рывком
				...
				return [[
					...
					он не угодил тебе в живот. Это тебе почти удаётся, но меч оставляет у тебя
					на боку кровавую полосу. От боли ты падаешь на землю.
					Под улюлюканье толпы урук-хай приходят новая вспышка боли и темнота.
				]]
			end
		end;
	end;
	...
	tak = function()
		if not _goblin_dead then
			...
			return [[
				...
				это только злит чудовище ещё больше.
				^
...
pit_spear = obj {
	...
	tak = function()
		...
		return [[
			...
			^
			Последнее, что ты слышишь -- как челюсти одичалого смыкаются
			у тебя на горле. Потом наступает темнота.
		]]
	end;
}
...
pit_shield = obj {
	...
	inv = [[
		...
		но кажется прочным. Под вмятинами можно разглядеть какой-то герб --
...
pit_wild = obj {
	...
	act = [[
		...
		Мощные руки ничуть не уступают по длине ногам и больше похожи на когтистые лапы.
	]];
	...
	used = function(self, what)
		if what == pit_fists then
			...
			return [[
				...
				Одичалый нависает над тобой, издавая победный рёв. Будь у тебя меч,
				ты мог бы попробовать вогнать его под ребро орка. Но меча у тебя нет.
				В отчаянии ты пинаешь тварь в пах, и чудовище с воем обрушивается
				на тебя, накрыв своей тушей.
				^
				Последнее, что ты слышишь -- как челюсти одичалого смыкаются
				у тебя на горле. Потом наступает темнота.
			]]
		...
		if (what == pit_sword) or (what == pit_axe) then
			walk 'tract_camp'
			return [[
				Одичалый обрушивает на тебя град ударов, разбивая твой щит в труху.
				В глазах темнеет. Собирав все свои силы, ты бросаешься куда-то вниз и вперёд и чудом
				...
				глубокие раны заставляют одичалого с рёвом рухнуть на землю.
				Пошатываясь, ты забираешься чудовищу на спину и парой ударов
				перерубаешь позвоночник. Предстмертные конвульсии овладевают тушей орка.
				В исступлении ты продолжаешь кромсать плоть, пока голова одичалого
				окончательно не отделяется от тела. Изнемогая, ты падаешь рядом с трупом.
				^
				Вокруг тебя лучи закатного солнца прорезают столбы пыли, окрашивая
				...
				банды. Но, к своему удивлению, ты обнаруживаешь, что ваш бой
				с одичалым проходил в молчании. Сейчас, когда кровь перестала бешено
				стучать в висках, а судороги оставили тело чудовища, тебе удаётся услышать
				ещё один звук -- скрип ветхих деревянных колёс по дорожной пыли.
				^
				Взгляды урук-хай направлены в сторону от ямы. Наконец спокойный
				...
				подхватывает твоё измождённое тело и выносит его обратно на площадь лагеря.
				На ней стоит огромная повозка, запряжённая лошадьми разной масти.
				Одна из дверей открывается, и под алый свет солнца спрыгивает человек.
				...
				солдата. Хотя он держит себя скорее как командир. Весьма молодой командир.
				Это заставляет тебя насторожиться ещё больше.
				^
				-- Мы вроде как договаривались действовать скрытно, -- продолжает человек.
				^
				-- Если посторонние уши или глаза могли узнать о присутствии урук-хай,
				...
				повторяющие одно и то же -- "колдун!". Штора обнажает тонкую белую руку.
				Даже издалека тебе удаётся увидеть на одном из длинных пальцев огромный
				сияющий перстень. Ты чувствуешь, как орки переводят взгляды на тебя.
				...
				чтобы посмотреть на тебя. Орки послушно исполняют приказ. Ты оказываешься
				...
				ещё одну дверцу. Какая-то неведомая сила, а может простая усталость, толкает тебя
