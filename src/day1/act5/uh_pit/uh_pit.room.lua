...
pit_axe = obj {
	...
	inv = [[
		...
		низкокачественного железа. Похоже на работу кузнеца.
	]];
}
...
pit_warren = obj {
	...
	dsc = function()
		if _warren_dead then
			return 'Залитое кровью тело Уорри валяется на земле.'
		end
...
pit_sword = obj {
	nam = 'Меч';
	dsc = [[
		Неподалёку от трупа Уорри лежит {меч}.
	]];
	tak = function()
		if not _goblin_dead then
			_goblin_dead = true
	...
	inv = [[
		...
		то становится понятным, почему его прозвали Ростовщиком.
	]];
}
