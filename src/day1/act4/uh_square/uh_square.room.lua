...
wood = obj {
	...
	inv = [[
		Ты внимательно обнюхиваешь доску. Чёрт знает, чем она пропитана.
	]];
}
...
uhs_totem = obj {
	...
	act = function()
		...
		if _uhs_position ~= 1 then
			return [[
				Вандал-тотем из твой деревни -- куча хлама с десятком
		...
		return [[
			...
			подобных проявлений орочьей культуры. Уруки весьма охотно занимаются
			...
			доски, тряпьё, оружие, части доспехов и любой хлам. Ну и, конечно, трупы
			...
			не менее характерные звуки. Ветер охотно разносит скрип, грохот и бряцанье
			по округе.
			^
	...
	used = function(self, what)
		...
		if what == wood then
			...
			return [[
				...
				^
				Ты подбираешь из обломков увесистый камень.
			]];
		end
	end;
}
	...
	used = function(self, what)
		...
		if what == arrow then
			...
			return [[
				...
				и с рёвом бросается на разорителя. В руках у разорителя оказывается нож,
				и тут же удар кулака сносит с его головы шлем. Размахивая ножом,
				разоритель кидается на огромного противника. Пара сцепляется и
				...
				^
				Драка продолжается ещё какое-то время. Как ни странно,
				победителем не выходит никто -- через пару минут оба урука
				валятся на землю, истекая кровью. Их товарищи, кажется,
				совсем не думают оказывать им первую помощь. Спустя ещё минуту
				вояка подходит, чтобы обыскать трупы.
			]]
		end
	end;
}
...
uhs_warrior = obj {
	...
	act = uhspick {
		[1] = [[
			Ты рассматриваешь скучающего вояку. Он прогуливается между шатров,
		...
		[5] = [[
			Ты внимательно рассматриваешь труп вояки. На шее зияет жуткая рана.
		]];
	};
	used = function(self, what)
		-- if player had stone, we assumed it's legal
		...
		if what == sword_shard then
			...
			return [[
				...
				пытаешься вспомнить, каково это -- драться с орком лицом к лицу.
