...
uhs_berserk = obj {
	...
	act = uhspick {
		...
		[2] = [[
			...
			в сталь рыцаря. Ты узнаёшь в этой махине из тугих мышц берсерка --
			самого яростного и отчаянного воина, которого можно встречтить в
			банде урук-хай.
			^
	...
	used = function(self, what)
		...
		if what == arrow then
			...
			return [[
				...
				^
				Спустя пару минут вояка подходит, чтобы обыскать трупы.
			]]
		end
	end;
}
...
uhs_devastator = obj {
	...
	dsc = uhspick {
		...
		[4] = [[
			Где-то под тушей берсерка лежит {труп разорителя}. Рядом валяется его
			рогатый шлем.
		]];
	};
	act = uhspick {
		[1] = [[
			Орк то и дело дёргается и что-то бормочет сквозь сон.
		]];
		[2] = [[
		...
		[4] = [[
			Ты видишь только руку разорителя, которая всё ещё крепко сжимает нож.
		]];
	};
	used = function(self, what)
