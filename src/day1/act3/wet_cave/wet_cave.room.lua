...
local totem_hint = [[
	Ты прикасаешься к тотему, и тишину пещерены нарушает скрипучий и в тоже
	время визгливый голос.
	^
	"
	...
	пленник обернулся тюремщиком, и нас призвали назад. Когда глубины
	пожирал огонь, а небеса разрывали древние крылья, мы снова пришли, с
	...
	...В четвёртый раз, но с моря пришли беглецы..."
    ^
    ^ 
	Разгадки тотемов относительно подземной хронологии, а не земной. Т.е. снег
	на тотеме будет обозначать Сагу о леденящем ужасе, а не эпоху зверя.
]];
	...
	if table.maxn(_totem_seq) == 4 then
		...
		if valid then
			...
			return [[
				...
				^
				"Теперь мы ждём новый приказ..."
			]];
		else
			_totem_seq = {};
			return [[
				Ты прикладываешь амулет к тотему. Все четыре тотема отзываются
				коротким низким грохотом и больше ничего.
			]];
		end
	else
		return [[
